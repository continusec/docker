...
// runInsertLayerCommand cherry-picks a diffID on top of the container that we're building
func (b *Builder) runInsertLayerCommand(diffID string, cmdName string) error {
	...
	return b.commit(container.ID, cmd, fmt.Sprintf("%s producing layer %s", cmdName, srcPath))
}
